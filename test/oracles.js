...
contract('Oracles', async (accounts) => {
  ...
  it('can register flight', async() => {
    ...
    let flight = "FlightOne"
    let timestamp = Math.floor(Date.now() / 1000);

    await config.flightSuretyApp.registerFlight(flight, timestamp, { from: firstAirline });
  ...
  it('can request flight status', async () => {
    ...
    await config.flightSuretyApp.fetchFlightStatus(firstAirline, flightName, timestamp);
    // ACT
    ...
    for(let a=10; a<TEST_ORACLES_COUNT; a++) {
      ...
      for(let idx=0;idx<3;idx++) {
        ...
        try {
          ...
          await config.flightSuretyApp.submitOracleResponse(oracleIndexes[idx], firstAirline, flightName, timestamp, STATUS_CODE_ON_TIME, { from: accounts[a] });
